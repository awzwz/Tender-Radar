    ...
    return (
      <SpecNormShell backHref="/specnorm/lots" backLabel="Список" title="Tender Analysis">
        <div className="flex flex-col items-center justify-center py-24 text-white/40">
          ...
          <div className="text-sm">Тендер {tenderId} не найден</div>
        </div>
      </SpecNormShell>
    );
  }
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {item.overallRiskTier === "HIGH" && (
                  <span className="rounded-full border border-rose-500/30 bg-rose-500/15 px-2.5 py-0.5 text-[10px] font-bold text-rose-300 animate-pulse">
                    HIGH RISK
                  </span>
                )}
              </div>
              <h1 className="mt-2 text-lg font-bold text-white leading-snug">{item.titleRu}</h1>
              ...
              {item.supplierName && (
                <div className="mt-1 text-sm text-white/50">
                  ...
                  Поставщик: <span className="text-white/70">{item.supplierName}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col items-end gap-3 flex-shrink-0">
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-center">
                  <div className="text-[10px] text-white/50 mb-1">Spec Deviation</div>
                  <RiskBadge score={item.specDeviationScore} size="lg" />
                  <div className="mt-2 w-32"><SeverityBar value={item.specDeviationScore} /></div>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-center">
                  <div className="text-[10px] text-white/50 mb-1">Doc Compliance</div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-indigo-500/30 bg-indigo-500/10 px-3 py-2 text-xs text-indigo-200 hover:bg-indigo-500/20 transition"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-200 hover:bg-amber-500/20 transition"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-xs text-cyan-200 hover:bg-cyan-500/20 transition"
                >
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-white">Проверка Заказчика</span>
              <span className="text-xs text-white/40">(Spec vs Norm)</span>
            </div>

            {/* Summary */}
            {(item.summary || []).length > 0 && (
              <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                <div className="text-xs font-semibold text-white/70 mb-2">Выводы</div>
            ...
            {(item.activeFlags || []).length > 0 && (
              <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                <div className="text-xs font-semibold text-white/70 mb-2">Сработавшие флаги</div>
            ...
            {comparisonRows.length > 0 && (
              <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                <div className="text-xs font-semibold text-white/70 mb-2">Extracted vs Norm</div>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {comparisonRows.map((row) => (
                        <tr key={row.key} className={`border-b border-white/5 ${row.strict ? "bg-rose-500/5" : ""}`}>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {row.tooltip && (
                                <span className="group relative">
                                  ...
                                  <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block whitespace-nowrap rounded-lg border border-white/10 bg-slate-900 px-2 py-1 text-[10px] text-white/70 shadow-lg z-10">
                                    {row.tooltip}
                                  </span>
                                </span>
                              )}
                          ...
                          <td className={`px-3 py-2.5 font-mono ${row.strict ? "text-rose-300 font-bold" : "text-white/80"}`}>
                            {row.ext !== undefined ? String(row.ext) : "—"}
                          </td>
                          <td className="px-3 py-2.5 font-mono text-white/60">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-white">Проверка Поставщика</span>
              <span className="text-xs text-white/40">(LLM Document Check)</span>
            </div>
            ...
            {violations.length > 0 ? (
              <div className="space-y-2">
                {violations.map((v, i) => (
                  <div key={i} className={`rounded-2xl border p-4 ${SEVERITY_STYLES[v.severity as Severity] ?? SEVERITY_STYLES.MEDIUM}`}>
                    <div className="flex items-center justify-between mb-2">
                      <ViolationChip type={v.type as any} severity={v.severity as Severity} />
                      <span className={`text-[10px] font-bold ${
                        v.severity === "HIGH" ? "text-rose-300" : v.severity === "MEDIUM" ? "text-amber-300" : "text-yellow-300"
                      }`}>
                        {v.severity}
                      </span>
                    </div>
                    <p className="text-sm text-white/80 mb-2">{v.description}</p>
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                      <div className="rounded-xl border border-white/10 bg-white/5 p-2">
                        <div className="text-[10px] text-white/50 mb-0.5">Требовалось:</div>
                        <div className="text-xs text-white/80">{v.requirement}</div>
                      </div>
                      <div className="rounded-xl border border-white/10 bg-white/5 p-2">
                        <div className="text-[10px] text-white/50 mb-0.5">Предоставлено:</div>
                        <div className="text-xs text-white/80">{v.provided}</div>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
                ...
                <div className="text-sm font-semibold text-emerald-200">Нарушений не выявлено</div>
                <div className="text-xs text-white/50 mt-1">Документы поставщика соответствуют требованиям техспецификации</div>
              </div>
            )}
            ...
            {item.llmAnalysis && (
              <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-semibold text-indigo-200">AI Анализ (OpenAI)</span>
                </div>
                <p className="text-sm text-white/80 leading-relaxed">{item.llmAnalysis}</p>
                <div className="mt-2 text-[10px] text-white/30">gpt-4o-mini · LLM-based document verification</div>
              </div>
            )}
            ...
            {item.supplierName && (
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="text-xs font-semibold text-white/70 mb-2">Информация о поставщике</div>
                <div className="text-sm text-white/80">{item.supplierName}</div>
                <div className="mt-2 text-xs text-white/50">
                  Doc Compliance Score: <span className="font-mono text-white/70">{item.docComplianceScore ?? 100}%</span>
                </div>
              </div>
            )}
          </div>
