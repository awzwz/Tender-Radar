      ...
      <div className="rounded-3xl border border-white/10 bg-slate-950/40 p-4 space-y-4">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-right flex-shrink-0">
            ...
            <div className="mt-1 text-[11px] text-white/40">Индикаторов: {triggeredFlags.length}/31</div>
            {/* Risk breakdown gauges */}
            <div className="mt-3 space-y-1.5 text-left">
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-white/50">Rules</span>
                <span className="text-white/80 font-mono">{risk.score_rules != null ? Math.round(risk.score_rules) : '—'}</span>
              </div>
              ...
              <div className="flex items-center justify-between text-[11px]">
                ...
                <span className="text-white/80 font-mono">{risk.score_ml != null ? (risk.score_ml * 100).toFixed(0) + '%' : '—'}</span>
              </div>
              <div className="h-1 w-full rounded-full bg-white/10 overflow-hidden">
              ...
              <div className="flex items-center justify-between text-[11px]">
                ...
                <span className="text-white/80 font-mono">{risk.score_final != null ? Math.round(risk.score_final) : '—'}</span>
              </div>
              <div className="h-1 w-full rounded-full bg-white/10 overflow-hidden">
            ...
            <div className="mt-3 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-3">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-indigo-200">AI Explanation</span>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  {explanation?.cached && (
                    <button
                      ...
                      disabled={explainLoading}
                      className="rounded-xl border border-indigo-500/30 px-2 py-1.5 text-xs text-indigo-300 hover:bg-indigo-500/10 transition disabled:opacity-50"
              ...
              {explanation && (
                <div className="space-y-2">
                  {explanation.error && !explanation.explanation && (
                    <div className="text-xs text-rose-300">{explanation.error}</div>
                  )}
                  {explanation.eligible === false && (
                    <div className="text-xs text-white/50">Объяснение недоступно: {explanation.reason}</div>
                  )}
                  ...
                  {explanation.spec_analysis && (
                    <div className={`rounded-xl border p-3 ${explanation.spec_analysis.risky ? "border-amber-500/30 bg-amber-500/10" : "border-emerald-500/20 bg-emerald-500/5"}`}>
                      <div className={`text-xs font-semibold mb-1 ${explanation.spec_analysis.risky ? "text-amber-200" : "text-emerald-200"}`}>
                        {explanation.spec_analysis.risky ? "⚠️ Анализ спецификаций: РИСК" : "✓ Анализ спецификаций: низкий риск"}
                      </div>
                      <p className="text-sm text-white/80">{explanation.spec_analysis.reasoning}</p>
                    </div>
                  )}
                  {explanation.checklist && explanation.checklist.length > 0 && (
                    <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3">
                      <div className="text-xs font-semibold text-emerald-200 mb-1">✅ Что проверить</div>
                      <ul className="list-disc pl-4 text-sm text-white/80 space-y-1">
    ...
    <div className="min-h-screen" style={{ background: "#020817", color: "#f1f5f9" }}>
      ...
      {onboardingOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="mx-4 max-w-md rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Sparkles className="h-5 w-5 text-indigo-300" />
                <h2 className="text-lg font-semibold text-white">Добро пожаловать в Tender Radar</h2>
              </div>
              <button onClick={closeOnboarding} className="rounded-xl p-2 text-white/50 hover:bg-white/10 hover:text-white transition">
                <X className="h-5 w-5" />
              </button>
            </div>
            <ul className="space-y-3 text-sm text-white/80 mb-6">
              <li className="flex gap-2"><ShieldAlert className="h-4 w-4 flex-shrink-0 text-indigo-300 mt-0.5" /><span><strong>Risk Score 0–100</strong> — общий уровень риска (LOW/MEDIUM/HIGH)</span></li>
              <li className="flex gap-2"><FileText className="h-4 w-4 flex-shrink-0 text-indigo-300 mt-0.5" /><span><strong>Lot Detail</strong> — карточка лота с индикаторами и AI-объяснением</span></li>
              <li className="flex gap-2"><Bot className="h-4 w-4 flex-shrink-0 text-indigo-300 mt-0.5" /><span><strong>Объяснить</strong> — AI анализирует риски и манипулятивные спецификации</span></li>
              <li className="flex gap-2"><ListFilter className="h-4 w-4 flex-shrink-0 text-indigo-300 mt-0.5" /><span><strong>Фильтры</strong> — min risk, уровень, поиск по номеру/заказчику</span></li>
            </ul>
            <div className="flex gap-2">
              ...
              <button onClick={closeOnboarding} className="rounded-xl border border-white/20 px-4 py-2.5 text-sm text-white/80 hover:bg-white/10 transition">
                Пропустить
              </button>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-5">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={runDemo}
              className="rounded-2xl border border-indigo-500/30 bg-indigo-500/10 px-3 py-2 text-xs hover:bg-indigo-500/20 transition text-indigo-200"
            >
